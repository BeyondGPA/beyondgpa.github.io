...
export class Visual4Component implements AfterViewInit {
  ...
  renderBoxPlot(data: any[]): void {
    ...
    let labels = grouped.map(([g]) => g.toFixed(1));

    // Sort labels and remove the 4.5 value
    labels = labels.filter(label => label !== '4.5').sort((a, b) => parseFloat(a) - parseFloat(b));

    const summary = grouped
        .filter(([gpa]) => labels.includes(gpa.toFixed(1))) // Exclude 4.5 from summary
        .map(([gpa, records]) => {
    ...
    const y = d3.scaleLinear()
        .domain([0, 5]) // Set y-axis domain to round numbers
        .range([height, 0]);
    ...
    svg.append('g')
        .call(d3.axisLeft(y).ticks(6)); // Ensure y-axis ticks are rounded
